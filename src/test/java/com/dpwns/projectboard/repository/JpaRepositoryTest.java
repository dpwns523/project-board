...
class JpaRepositoryTest {
    ...
    void givenParentCommentId_whenSelecting_thenReturnsChildComments(){
        ...
        assertThat(parentComment).get()
                ...
                .extracting("childComments", InstanceOfAssertFactories.COLLECTION)  // child 댓글 안으로 들어가는 것
                .hasSize(4);
    }

    @DisplayName("댓글 삭제와 대댓글 전체 연동 삭제 테스트 - 댓글 ID + 유저 ID")
    @Test
    void givenArticleCommentIdHavingChildCommentsAndUserId_whenDeletingParentComment_thenDeleteEveryComment(){
        // Given
        long previousArticleCommentCount = articleCommentRepository.count();
    ...
    void givenParentComment_whenSaving_thenInsertsChildComment(){
        ...
        articleCommentRepository.flush();   // 영속성 더티체크
    ...
    @DisplayName("댓글 삭제와 대댓글 전체 연동 삭제 테스트")  // 기본 JPARepository를 활용한 테스트
    @Test
    ...
    @DisplayName("댓글 삭제와 대댓글 전체 연동 삭제 테스트 - 댓글 ID + 유저 ID") // 실제 사용하는 비즈니스 로직 테스트
    @Test
